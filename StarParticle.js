	...
	this.draw = function() {
		if (this.color != null ) {
			...
			if (currentMode === 'lines') {
                ...
                rectMode(CENTER);
                for (let i = 0; i < 3; i++) {
                    let r = s * random(0.5, 1.5) * this.scale;
                    rect(this.position.x + random(-this.spread, this.spread), this.position.y + random(-this.spread, this.spread), r, r);
                }
            }
            else if (currentMode === 'crosses') {
                strokeWeight(s * 0.5);
                push();
                translate(this.position.x + random(-this.spread, this.spread), this.position.y + random(-this.spread, this.spread));
                let r = s * random(0.4, 1.0) * this.scale;
                line(-r, -r, r, r);
                line(r, -r, -r, r);
                pop();
            }
				...
				for (let i = 0; i < 3; i++) {
					circle(this.position.x + random(-this.spread, this.spread), this.position.y + random(-this.spread, this.spread), s * random(0.5, 1.5) * this.scale)
				}
			}
		}
		this.updatePrevious();
